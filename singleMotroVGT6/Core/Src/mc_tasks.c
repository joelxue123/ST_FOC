...
__weak void MCboot( MCI_Handle_t* pMCIList[NBR_OF_MOTORS],MCT_Handle_t* pMCTList[NBR_OF_MOTORS] )
{
  ...
  bMCBootCompleted = 0;//初始化完成标准清零 未初始化完成
  pCLM[M1] = &CircleLimitationM1;//极限圆变量初始化

  /**********************************************************/
  /*    PWM and current sensing component initialization    */
  /**********************************************************/
  pwmcHandle[M1] = &PWM_Handle_M1._Super;//电机控制相关函数变量初始化
  R3_2_Init(&PWM_Handle_M1);//三电阻2 adc采样功能初始化
...
/* USER CODE END MC_Scheduler 0 */
  ...
  if (bMCBootCompleted == 1)
  {
    ...
    else
    {
      ...
      hMFTaskCounterM1 = MF_TASK_OCCURENCE_TICKS;//重新赋值计数500us执行一次
    }
...
__weak void TSK_MediumFrequencyTaskM1(void)
{
  ...
  State_t StateM1;//电机运行状态状态
  int16_t wAux = 0;
//计算速度 并返回速度是否稳定的状态
  bool IsSpeedReliable = STO_PLL_CalcAvrgMecSpeedUnit( &STO_PLL_M1, &wAux );
  PQD_CalcElMotorPower( pMPM[M1] );//计算电机功率
  //获得当前电机状态
  ...
  case IDLE_START:
    ...
    R3_2_TurnOnLowSides( pwmcHandle[M1] );//开三个下管
    TSK_SetChargeBootCapDelayM1( CHARGE_BOOT_CAP_TICKS );//CHARGE_BOOT_CAP_TICKS：充电时间T 需要在程序中执行多少次
    STM_NextState( &STM[M1], CHARGE_BOOT_CAP );
    break;

  case CHARGE_BOOT_CAP:
    if ( TSK_ChargeBootCapDelayHasElapsedM1() )//判断是否是首次启动是否需要电容充电
    {
			//初始化读取下管偏执
  ...
  case CLEAR:
    ...
    FOCVars[M1].bDriveInput = EXTERNAL;//电流给定默认外部给定
  ...
  case START:
    {
      ...
      else
      {
        ...
        IqdRef.q = STC_CalcTorqueReference( pSTC[M1] );//iq给定 //pSTC[M1] = &SpeednTorqCtrlM1;
        IqdRef.d = FOCVars[M1].UserIdref;//id给定一般给定为0
        /* Iqd reference current used by the High Frequency Loop to generate the PWM output */
        FOCVars[M1].Iqdref = IqdRef;//给定到foc算法中
      }
				//计算机械速度
      (void) VSS_CalcAvrgMecSpeedUnit( &VirtualSpeedSensorM1, &hForcedMecSpeedUnit );

      /* check that startup stage where the observer has to be used has been reached */
      if (RUC_FirstAccelerationStageReached(&RevUpControlM1) == true)//判读启动周期是否满足
      {
        ObserverConverged = STO_PLL_IsObserverConverged( &STO_PLL_M1,hForcedMecSpeedUnit );//判断是sto否收敛？？？
        (void) VSS_SetStartTransition( &VirtualSpeedSensorM1, ObserverConverged );//开始切换 由强制启动切换到估算角度
      }

      if ( ObserverConverged )//观测器收敛
      {
        qd_t StatorCurrent = MCM_Park( FOCVars[M1].Ialphabeta, SPD_GetElAngle( &STO_PLL_M1._Super ) );

        /* Start switch over ramp. This ramp will transition from the revup to the closed loop FOC. */
        REMNG_Init( pREMNG[M1] );
				//为高频任务中SWITCH_OVER状态时使用
 ...
 /* only for sensor-less control */
	//观测器的速度相关变量赋值给SpeednTorqCtrl_Handle_t结构体
...
__weak void FOC_CalcCurrRef(uint8_t bMotor)
{
  ...
  {
    FOCVars[bMotor].hTeref = STC_CalcTorqueReference(pSTC[bMotor]);//给定值以斜坡的方式逐渐给到Iq
...
//上一次的ua和ub 赋值给滑膜算法
  ...
  if ( STM[M1].bState == SWITCH_OVER )//start-run状态的过度状态  
  {
//						//为高频任务中SWITCH_OVER状态时使用
//        REMNG_ExecRamp( pREMNG[M1], FOCVars[M1].Iqdref.q, 0 );
//        REMNG_ExecRamp( pREMNG[M1], StatorCurrent.q, TRANSITION_DURATION );
    if (!REMNG_RampCompleted(pREMNG[M1]))//读取计步次数值 不为0 斜坡函数没有执行完成
    {
      FOCVars[M1].Iqdref.q = REMNG_Calc(pREMNG[M1]);//斜坡函数
    }
  }
  /* USER CODE BEGIN HighFrequencyTask SINGLEDRIVE_1 */

  /* USER CODE END HighFrequencyTask SINGLEDRIVE_1 */
  hFOCreturn = FOC_CurrControllerM1();//FOC 核心算法
  /* USER CODE BEGIN HighFrequencyTask SINGLEDRIVE_2 */

  /* USER CODE END HighFrequencyTask SINGLEDRIVE_2 */
	//foc算法运行时间大于每次计数上溢中断时间
  if(hFOCreturn == MC_FOC_DURATION)// 发生foc算法没有运行完成定时器1上溢中断再次发生会使算法运行错误 电机运行不正常
  {
    STM_FaultProcessing(&STM[M1], MC_FOC_DURATION, 0);//状态机报错
  }
  else//正常运行 foc算法运行时间小于每次溢出中断时间
  {
    bool IsAccelerationStageReached = RUC_FirstAccelerationStageReached(&RevUpControlM1);//RUN_Exe函数中执行的次数大于限定的次数任务首次加速没有成功
		//将iaib赋值给sto算法结构体变量
    ...
    STO_PLL_CalcAvrgElSpeedDpp (&STO_PLL_M1); /*  Only in case of Sensor-less */
	 if (IsAccelerationStageReached == false)//运行RUN_Exe函数 没有在规定的限制内启动成功
    {
...
inline uint16_t FOC_CurrControllerM1(void)
{
  ...
  speedHandle = STC_GetSpeedSensor(pSTC[M1]);//获取速度传感器结构体中所以变量
  hElAngle = SPD_GetElAngle(speedHandle);//得到角度值
  hElAngle += SPD_GetInstElSpeedDpp(speedHandle)*PARK_ANGLE_COMPENSATION_FACTOR;//读取瞬时角度
  PWMC_GetPhaseCurrents(pwmcHandle[M1], &Iab);//根据扇区读取两相电流
  Ialphabeta = MCM_Clarke(Iab);// 3/2变换输出ia ib 理解：原则是以一相作为参考波形作为基准 变成两相 
	//例如说 通常以0 作为参考 定数字 1 2 3 这时候我们以1 作为参 那么 就只有2 和 3 
	
  Iqd = MCM_Park(Ialphabeta, hElAngle);//2/2变换 理解：参考面也是以正选波来变化 那么id iq 就变成的直流
	//从这里可以给定 用户需要的 idref 和iqref PI调节后输出 Vd Vq
  Vqd.q = PI_Controller(pPIDIq[M1],
            (int32_t)(FOCVars[M1].Iqdref.q) - Iqd.q);

  Vqd.d = PI_Controller(pPIDId[M1],
            (int32_t)(FOCVars[M1].Iqdref.d) - Iqd.d);
// Vq 极限圆 用来限制最大调制的输出 防止采样窗口太小无法采集到电流
  Vqd = Circle_Limitation(pCLM[M1], Vqd);
  hElAngle += SPD_GetInstElSpeedDpp(speedHandle)*REV_PARK_ANGLE_COMPENSATION_FACTOR;//??????
  Valphabeta = MCM_Rev_Park(Vqd, hElAngle);//反变换park
  hCodeError = PWMC_SetPhaseVoltage(pwmcHandle[M1], Valphabeta);//设置相电压
	//赋值给电机结构体变量
  FOCVars[M1].Vqd = Vqd;
  FOCVars[M1].Iab = Iab;
