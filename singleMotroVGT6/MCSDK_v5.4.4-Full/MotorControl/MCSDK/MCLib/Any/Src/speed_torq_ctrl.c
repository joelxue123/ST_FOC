...
//STC_ExecRamp( pHandle->pSTC, pHandle->hFinalSpeed, pHandle->hDurationms );//MCI_Handle_t
...
__weak bool STC_ExecRamp( SpeednTorqCtrl_Handle_t * pHandle, int16_t hTargetFinal, uint32_t hDurationms )
{
  ...
  if ( pHandle->Mode == STC_TORQUE_MODE )//力矩模式
  {
    hCurrentReference = STC_GetTorqueRef( pHandle );//获取上次用户力矩给定值
  ...
  if ( AllowedRange == true )
  {
    ...
    else
    {
      ...
      the ramp. */
			//计算斜坡时间hDurationms*执行频率 得到这个斜坡执行多少次到达目标
      wAux = ( uint32_t )hDurationms * ( uint32_t )pHandle->STCFrequencyHz;//1000 
      wAux /= 1000u;//统一单位 转换成ms
      ...
      wAux1 = ( ( int32_t )hTargetFinal - ( int32_t )hCurrentReference ) * 65536;//斜坡给定
      wAux1 /= ( int32_t )pHandle->RampRemainingStep;
      pHandle->IncDecAmount = wAux1;//增量值
    }
  }
...
//是速度模式还是力矩模式
  ...
  if ( pHandle->RampRemainingStep > 1u )
  {
    ...
    wCurrentReference += pHandle->IncDecAmount;//每一次的增量  ,函数RUC_Exec中计算得出
  ...
  else if ( pHandle->RampRemainingStep == 1u )
  {
    ...
    wCurrentReference = ( int32_t )pHandle->TargetFinal * 65536;//最后一步 给定最大值
    pHandle->RampRemainingStep = 0u;
  }
  else
  {
...
//速度模式
  if ( pHandle->Mode == STC_SPEED_MODE )
  {
    ...
    hMeasuredSpeed = SPD_GetAvrgMecSpeedUnit( pHandle->SPD );//得到平均速度 
