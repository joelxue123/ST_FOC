...
*/
//VSS_CalcElAngle(&VirtualSpeedSensorM1,&hObsAngle); //hObsAngle观测的角度
__weak int16_t VSS_CalcElAngle( VirtualSpeedSensor_Handle_t * pHandle, void * pInputVars_str )
{
  ...
  if ( pHandle->bCopyObserver == true )//逆风启动时使用
  {
  ...
  else
  {
    ...
    if ( pHandle->bTransitionStarted == true )//观测器收敛时开始切换大sto观测角度 
    {
      ...
      {
        pHandle->hTransitionRemainingSteps--;//角度切换剩余计数--
       //将强制启动角度逐渐趋近观测器角度
        if ( pHandle->_Super.hElSpeedDpp >= 0 )
        {
          hAngleDiff = *( int16_t * )pInputVars_str - pHandle->hElAngleAccu;//观测器的角度和强制启动角度的差值
        }
        else
        {
...
__weak bool VSS_CalcAvrgMecSpeedUnit( VirtualSpeedSensor_Handle_t * pHandle, int16_t * hMecSpeedUnit )
{
  ...
  {
    pHandle->wElSpeedDpp32 += pHandle->wElAccDppP32;//加速度的积分是速度
    pHandle->_Super.hElSpeedDpp = ( int16_t )( pHandle->wElSpeedDpp32 / 65536 );

    /* Convert dpp into MecUnit */
		//1dpp = (1/F(foc)) * ((2pi/65535) //一个计算周期内运动的角度=速度 将dpp单位转换成机械速度  
...
__weak void VSS_SetMecAngle( VirtualSpeedSensor_Handle_t * pHandle, int16_t hMecAngle )
{
  ...
  pHandle->_Super.hMecAngle = pHandle->hElAngleAccu / ( int16_t )pHandle->_Super.bElToMecRatio;//电气角度除以极对数就是机械角度
                              ...
                              uint16_t hDurationms )
{
  ...
  {
    if ( hDurationms == 0u )//首次启动
    {
      pHandle->_Super.hAvrMecSpeedUnit = hFinalMecSpeedUnit;
//#define DPP_CONV_FACTOR (65536/PWM_FREQ_SCALING)//65535/1
			//hFinalMecSpeedUnit =  1s钟多少度,
			//放大十倍乘SPEED_UNIT   
			//1ddp = 2pi*F(foc)/65535  
    ...
    else
    {
      ...
      hCurrentMecSpeedDpp = pHandle->_Super.hElSpeedDpp /
                            ( int16_t )pHandle->_Super.bElToMecRatio;//机械角度 = 电气角度/极对数
			//用户给定的速度转换成ddp的形式
