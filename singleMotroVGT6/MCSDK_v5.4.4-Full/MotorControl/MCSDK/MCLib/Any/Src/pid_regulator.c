...
//防止矛盾发生 防止溢出 pHandle->wIntegralTerm 和 wIntegral_Term 都大于0的时候，累加和wIntegral_sum_temp小于0说明溢出了
...
#endif
//防止积分饱和
  if ( wOutput_32 > hUpperOutputLimit )//输出已经到达上极限
  {

    wDischarge = hUpperOutputLimit - wOutput_32;//wDischarge为小于0的数
    wOutput_32 = hUpperOutputLimit;
  }
  else if ( wOutput_32 < hLowerOutputLimit )//输出已经到达下极限
  {

    wDischarge = hLowerOutputLimit - wOutput_32;////wDischarge为大于0的数
    wOutput_32 = hLowerOutputLimit;
  }
  else { /* Nothing to do here */ }

  pHandle->wIntegralTerm += wDischarge;//对积分和进行增减 在下次运行pi的时候反应 最大超调点到目标值的距离会减小，这样减小了超调量，PI反应会更迅速
